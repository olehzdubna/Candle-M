...
CandleConnection::Type frmSettings::connType() {
   if(ui->rbSerial->isChecked())
       return CandleConnection::CONN_SERIAL;
   else
       if(ui->rbTcpIp->isChecked())
           return CandleConnection::CONN_TCPIP;
   return CandleConnection::CONN_NA;
}

void frmSettings::setConnType(int ct) {
    if(ct == CandleConnection::CONN_SERIAL)
        ui->rbSerial->setChecked(1);
    else
        if(ct == CandleConnection::CONN_TCPIP)
            ui->rbTcpIp->setChecked(1);
}
...
QString frmSettings::tcpHost() {
    return ui->leIPAddr->text();
}

void frmSettings::setTcpHost(QString host) {
     ui->leIPAddr->setText(host);
}

int frmSettings::tcpPort() {
    return ui->leIPPort->text().toUInt();
}

void frmSettings::setTcpPort(uint16_t port) {
    ui->leIPPort->setText(QString::number(port));
}
