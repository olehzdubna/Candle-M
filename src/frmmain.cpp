...
frmMain::frmMain(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::frmMain),
    m_connection((QObject*)this)
{
    // Loading settings
    m_settingsFileName = qApp->applicationDirPath() + "/settings.ini";
    preloadSettings();

    m_settings = new frmSettings((QWidget*)this);
...
//    m_machine = (Machine*) new GrblMachine(this, ui, m_connection);
    m_machine = (Machine*) new MarlinMachine(this, ui, m_connection);
    ...
    connect(ui->slbSpindle, &SliderBox::valueUserChanged, [=] {m_machine->updateSpindleSpeed() = true;});
 ...
 //   connect(&m_connection, SIGNAL(tcpReadyRead()), this, SLOT(onSerialPortReadyRead()), Qt::QueuedConnection);
    m_connection.registerReadHandler(this, SLOT(onCommReadyRead()));
//    connect(&m_connection, SIGNAL(error(QSerialPort::SerialPortError)), this, SLOT(onSerialPortError(QSerialPort::SerialPortError)));
    m_connection.registerErrorHandler(this, SLOT(onCommError(QSerialPort::SerialPortError)));
...
void frmMain::loadSettings()
{
    ...
    m_settings->setConnType(set.value("connType").toInt());
    m_connection.setConnType(m_settings->connType());
    m_settings->setPort(set.value("port").toString());
    m_connection.setPortName(m_settings->port());
    m_settings->setBaud(set.value("baud").toInt());
    m_connection.setBaudRate(m_settings->baud());
    m_settings->setTcpHost(set.value("tcpHost").toString());
    m_connection.setTcpHost(m_settings->tcpHost());
    m_settings->setTcpPort(set.value("tcpPort").toInt());
    m_connection.setTcpPort(m_settings->tcpPort());
    ...
    m_machine->storedX() = set.value("storedX", 0).toDouble();
    m_machine->storedY() = set.value("storedY", 0).toDouble();
    m_machine->storedZ() = set.value("storedZ", 0).toDouble();

    ui->cmdRestoreOrigin->setToolTip(QString(tr("Restore origin:\n%1, %2, %3")).arg(m_machine->storedX()).arg(m_machine->storedY()).arg(m_machine->storedZ()));
...
void frmMain::saveSettings()
{
    ...
    set.setValue("connType", m_settings->connType());
    set.setValue("tcpHost", m_settings->tcpHost());
    set.setValue("tcpPort", m_settings->tcpPort());
    ...
    set.setValue("storedX", m_machine->storedX());
    set.setValue("storedY", m_machine->storedY());
    set.setValue("storedZ", m_machine->storedZ());
...
void frmMain::updateControlsState() {
    bool portOpened = m_connection.isOpen();
    ...
    if(portOpened) {
        ...
        ui->pbConnect->setStyleSheet(QString("background-color: rgb(0, 255, 0); color: rgb(0, 0, 0);"));
    } else {
        ui->pbConnect->setText(tr("Connect"));
        ui->pbConnect->setStyleSheet(QString("background-color: rgb(255, 0, 0); color: rgb(0, 0, 0);"));
    }

    ui->grpControl->setEnabled(portOpened);
    ui->widgetUserCommands->setEnabled(portOpened && !m_machine->processingFile());
    ui->widgetSpindle->setEnabled(portOpened);
    ui->widgetJog->setEnabled(portOpened && !m_machine->processingFile());
...
//    ui->widgetFeed->setEnabled(!m_transferringFile);
    ...
    ui->chkTestMode->setEnabled(portOpened && !m_machine->processingFile());
    ui->cmdHome->setEnabled(!m_machine->processingFile());
    ui->cmdTouch->setEnabled(!m_machine->processingFile());
    ui->cmdZeroXY->setEnabled(!m_machine->processingFile());
    ui->cmdZeroZ->setEnabled(!m_machine->processingFile());
    ui->cmdRestoreOrigin->setEnabled(!m_machine->processingFile());
    ui->cmdSafePosition->setEnabled(!m_machine->processingFile());
    ui->cmdUnlock->setEnabled(!m_machine->processingFile());
    ui->cmdSpindle->setEnabled(!m_machine->processingFile());

    ui->actFileNew->setEnabled(!m_machine->processingFile());
    ui->actFileOpen->setEnabled(!m_machine->processingFile());
    ui->cmdFileOpen->setEnabled(!m_machine->processingFile());
    ui->cmdFileReset->setEnabled(!m_machine->processingFile() && m_programModel.rowCount() > 1);
    ui->cmdFileSend->setEnabled(portOpened && !m_machine->processingFile() && m_programModel.rowCount() > 1);
    ui->cmdFilePause->setEnabled(m_machine->processingFile() && !ui->chkTestMode->isChecked());
    ui->cmdFileAbort->setEnabled(m_machine->processingFile());
    ui->actFileOpen->setEnabled(!m_machine->processingFile());
    ui->mnuRecent->setEnabled(!m_machine->processingFile() && ((m_recentFiles.count() > 0 && !m_heightMapMode)
    ...
    ui->tblProgram->setEditTriggers(m_machine->processingFile() ? QAbstractItemView::NoEditTriggers :
    ...
    if (!m_machine->processingFile()) ui->chkKeyboardControl->setChecked(m_storedKeyboardControl);

#ifdef WINDOWS
    if (QSysInfo::windowsVersion() >= QSysInfo::WV_WINDOWS7) {
        if (!m_machine->processingFile() && m_taskBarProgress) m_taskBarProgress->hide();
    }
#endif
    ...
    ui->grpHeightMapSettings->setEnabled(!m_machine->processingFile() && !ui->chkKeyboardControl->isChecked());
    ...
    ui->widgetHeightMap->setEnabled(!m_machine->processingFile() && m_programModel.rowCount() > 1);
...
void frmMain::onCommError(int error)
{
    m_machine->onCommError(error);
}

void frmMain::onTimerConnection()
{
    m_machine->onTimerConnection();
}

void frmMain::onTimerStateQuery()
{
    m_machine->onTimerStateQuery();
}
...
void frmMain::onScroolBarAction(int action)
{
    ...
    if (m_machine->processingFile()) ui->chkAutoScroll->setChecked(false);
}
...
void frmMain::timerEvent(QTimerEvent *te)
{
    if (te->timerId() == m_timerToolAnimation.timerId()) {
        m_toolDrawer.rotate((m_machine->spindleCW() ? -40 : 40) * (double)(ui->slbSpindle->currentValue())
...
void frmMain::closeEvent(QCloseEvent *ce)
{
    ...
    if (m_machine->processingFile() && QMessageBox::warning(this, this->windowTitle(), tr("File sending in progress. Terminate and exit?"),
...
{
    if (m_machine->processingFile()) return;
...
void frmMain::on_cmdFileSend_clicked()
{
    ...
    if (!ui->chkTestMode->isChecked())
        m_machine->storeOffsets(); // Allready stored on check
    m_machine->storeParserState();
    ...
    qDebug("+++ frmMain::on_cmdFileSend_clicked - before sendNextFileCommands");

    m_machine->sendNextFileCommands();
}
...
void frmMain::onActSendFromLineTriggered()
{
    ...
    if (m_settings->autoLine()) {
        ...
        if (res == QMessageBox::Cancel) return;
            ...
            foreach (QString command, commands) {
                m_machine->sendCommand(command, -1, m_settings->showUICommands());
            }
        }
    }

    m_machine->resetFile(commandIndex);
    ...
    m_machine->fileCmdIndex(commandIndex);
...
{
    if (ui->tblProgram->selectionModel()->selectedRows().count() == 0 || m_machine->processingFile()) return;
...
{
    if (ui->tblProgram->selectionModel()->selectedRows().count() == 0 || m_machine->processingFile() ||
...
void frmMain::on_actServiceSettings_triggered()
{
    if (m_settings->exec()) {
        ...
        qDebug() << "Connection Type:" << (m_settings->connType() == CandleConnection::CONN_SERIAL ? "Serial" : "")
                 << (m_settings->connType() == CandleConnection::CONN_TCPIP ? "TCP/IP" : "");
        qDebug() << "Port:" << m_settings->port() << ", Baud:" << m_settings->baud();
        qDebug() << "TCP/IP Host:" << m_settings->tcpHost() << ", TCP/IP Port:" << m_settings->tcpPort();

        if(m_settings->connType() == CandleConnection::CONN_SERIAL) {
            if (m_settings->port() != "" && (m_settings->port() != m_connection.portName() ||
                                           m_settings->baud() != m_connection.baudRate())) {
                m_connection.setPortName(m_settings->port());
                m_connection.setBaudRate(m_settings->baud());
            }
        } else
            if(m_settings->connType() == CandleConnection::CONN_TCPIP) {

                if (m_settings->tcpHost() != "" && (m_settings->tcpHost() != m_connection.tcpHost() ||
                                               m_settings->tcpHost() != m_connection.tcpPort())) {
...
//    m_homing = true;
    ...
    foreach (QString cmd, list) {
        m_machine->sendCommand(cmd.trimmed(), -1, m_settings->showUICommands());
    }
}
...
{
    m_machine->cmdSpindle(checked);
}

void frmMain::on_chkTestMode_clicked(bool checked)
{
    m_machine->testMode(checked);
}
...
bool frmMain::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if (obj == this || obj == ui->tblProgram || obj == ui->cboJogStep || obj == ui->cboJogFeed) {
        ...
        if (!m_machine->processingFile() && ui->chkKeyboardControl->isChecked() &&
        ...
        if (event->type() == QEvent::KeyPress) {
            ...
            if (!m_machine->processingFile() && keyEvent->key() == Qt::Key_ScrollLock && obj == this) {
            ...
            if (obj == ui->tblProgram && m_machine->processingFile()) {
...
void frmMain::on_chkKeyboardControl_toggled(bool checked)
{
    ...
    m_machine->storeCoordinateSystem(checked);

    if (!m_machine->processingFile())
...
void frmMain::onCboCommandReturnPressed()
{
    ...
    m_machine->sendCommand(command, -1);
}
...
//        m_codeDrawer->updateData(); // Force update data to properly shadowing
    ...
    QList<LineSegment*> list = m_viewParser.getLineSegmentList();
    QList<int> indexes;
    for (int i = m_machine->lastDrawnLineIndex(); i < list.count(); i++) {
...
void frmMain::updateOverride(SliderBox *slider, int value, char command)
{
    ...
    if (slider->currentValue() < target) {
        m_connection.write(QByteArray(1, char(smallStep ? command + 2 : command)));
    } else if (slider->currentValue() > target) {
        m_connection.write(QByteArray(1, char(smallStep ? command + 3 : command + 1)));
    }
}
...
{
    m_machine->jogVector() += QVector3D(0, 1, 0);
    m_machine->jogStep();
}
...
{
    m_machine->jogVector() += QVector3D(1, 0, 0);
    m_machine->jogStep();
}
...
{
    m_machine->jogVector() += QVector3D(0, 0, 1);
    m_machine->jogStep();
}
...
void frmMain::on_pbConnect_clicked()
{
    printf("+++ on_pbConnect_clicked\n");
